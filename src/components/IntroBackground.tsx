...
export default function IntroBackground() {
	return (
		<div className="pointer-events-none absolute inset-0 z-0 h-full w-full overflow-hidden" aria-hidden="true">
			...
			<div className="absolute -top-24 left-1/2 h-80 w-80 -translate-x-1/2 rounded-full bg-cyan-400/12 blur-2xl" />
			<div className="absolute -bottom-20 left-1/3 h-64 w-64 rounded-full bg-fuchsia-400/10 blur-2xl" />

			<motion.div
				initial={{ opacity: 0, scale: 0.96 }}
				animate={{ opacity: 1, scale: 1 }}
				transition={{ duration: 0.8, ease: 'easeOut' }}
				className="absolute left-1/2 top-1/2 h-[72vmin] w-[72vmin] min-h-[360px] min-w-[360px] max-h-[900px] max-w-[900px] -translate-x-1/2 -translate-y-1/2 will-change-transform"
			>
				<DotLottieReact
					src={animationSrc}
					autoplay
